...
export default function SplitView({ document, onClose }) {
    ...
    const validateConfiguration = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Dispatch event to notify other components (e.g., Carousel)
    ...
    // Voice configuration navigation - activated by handsfree_mode
    useEffect(() => {
        let voiceConfig = null;

        const initVoiceConfig = async () => {
            if (!config.handsfree_mode) {
                if (voiceConfig) {
                    voiceConfig.stopListening();
                    voiceConfig = null;
                }
                return;
            }
            ...
            try {
                const module = await import('../utils/voiceConfigNavigation.js');
                const VoiceConfigNavigation = module.default;
                voiceConfig = new VoiceConfigNavigation();

                voiceConfig.onStatus((status, data) => {
                    console.log('[SplitView] Voice status:', status, data);
                });

                voiceConfig.onResult((result) => {
                    console.log('[SplitView] Voice result:', result);
                    
                    if (result.success && result.config) {
                        // Update configuration with voice command result
                        setConfig(result.config);
                        
                        // Show feedback
                        alert(result.message);
                    } else {
                        alert(result.message || 'Commande vocale echouee');
                    }
                });

                voiceConfig.startListening();
                console.log('[SplitView] Voice config initialized and listening');
            } catch (error) {
                console.error('[SplitView] Error initializing voice config:', error);
            }
        };
                    ...
                    <div className="config-section">
                        ...
                        <div className="config-toggle-group">
                            ...
                            <label className="config-toggle">
                                <input 
                                    type="checkbox" 
                                    checked={config.handsfree_mode}
                                    onChange={(e) => handleConfigChange('handsfree_mode', e.target.checked)}
                                />
                                <span>Mode mains libres (commandes vocales)</span>
                            </label>
                        </div>
                    </div>
